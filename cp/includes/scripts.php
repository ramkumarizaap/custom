...
<?php  if($page=="discounts-add" || $page=="showroom-add" || $page=="reviews-add"){?>

<script src="<?php echo $homeurl;?>assets/js/bootstrap-datetimepicker.min.js"></script>   

<script type="text/javascript">
 $(".date-time").datetimepicker({sideBySide:true,format:"MM/DD/YYYY HH:mm", inline: true});
...
$(".resend_btn").click(function(){
  service_message("warning","Please Wait....");
  id = $(this).attr("data-id");
  $.ajax({
    type:"POST",
    url:"<?php echo $adminurl;?>ajax/resend_email.php",
    data:{id:id},
    dataType:'json',
    success:function(data)
    { 
...
$(".product_accents").change(function(){
  val = $(this).val();
  $(".product-accents-list").html("Please Wait....");
  $.ajax({
    type:"POST",
    url:"<?php echo $adminurl;?>property/accents-list.php",
    data:{id:val},
    success:function(data)
    {
      $(".product-accents-list").html(data);
    }
  });
});
...
<?php if($page == "jacket_lining_add"){ ?>
<script type="text/javascript">
$(".add_custom_color").click(function(){

  row='<tr>'+
        '<td><input type="text" class="form-control" name="image_name[]" placeholder="Lining Name"></td>'+
        '<td><textarea class="form-control" name="description[]" placeholder="Desc"></textarea></td>'+
        '<td><input type="text" class="form-control" name="price[]" placeholder="Price"></td>'+
        '<td><input type="file" name="col_large_img[]" class="form-control" placeholder="Large Image">'+
        '<td><input type="file" name="col_small_img[]" class="form-control" placeholder="Small Image">'+
        '<td><a href="javascript:void(0);" class="rem_col btn btn-danger btn-xs">'+'<i class="fa fa-trash"></i> Remove'+'</a>'+'</td>'+
      '</tr>';
      $(".color_table tbody").append(row);
      $(".my-color").minicolors();
});
...
<?php if($page == "showroom_template"){ ?>

<script type="text/javascript">

    $(".get_showroom_template").on('change',function(){
      val = $(this).val();
      $.ajax({
        type:"POST",
        url:"<?php echo $adminurl;?>ajax/get_showroom_template.php",
        data:{id:val},
        dataType:'json',
        success:function(data)
        {
          ...
          {
            tinyMCE.get('body_message').setContent(data.message.body_message, {format : 'raw'}); 
            tinyMCE.get('bottom_message').setContent(data.message.bottom_message, {format : 'raw'}); 
            // $("textarea[name='body_message']").val(data.message.body_message);
            // $("textarea[name='bottom_message']").val(data.message.bottom_message);
            $("input[name='email']").val(data.message.email);
          }
          else
          {
            tinyMCE.get('body_message').setContent('', {format : 'raw'}); 
            tinyMCE.get('bottom_message').setContent('', {format : 'raw'}); 
          }
        }
      });
  });
...
<?php } ?>
<script type="text/javascript">
  $("form#measureForm").submit(function(e){
    e.preventDefault();
    var err = "";var valid = true;
    ...
    if(form[3].value=="")
    {
      err += "Chest Field is required.\n";
      valid = false;
    }
    if(form[4].value=="")
    {
      err += "Abdomen Field is required.\n";
      valid = false;
    }
    if(form[5].value=="")
    {
      err += "Buttocks Field is required.\n";
      valid = false;
    }
    if(form[6].value=="")
    {
      err += "Hips Field is required.\n";
      valid = false;
    }
    if(!valid)
    {
    ...
    else
    {
      $.ajax({
        type:"POST",
        url:"<?php echo $adminurl;?>ajax/save_measurement.php",
        data:form,
        dataType:'json',
        success:function(data)
        {
...
<script type="text/javascript">
  function service_message(status='',msg='')
  {
  ...
  function modify_measurement()
  {
    $(".label-value,.save-btn").toggleClass("hide");
    $(".input-value").toggleClass("hide");
  }
</script>
<?php  if($page=="product-add"){?>
  <script type="text/javascript" src="<?php echo $homeurl;?>assets/js/default_properties.js"></script>
<?php
}?>
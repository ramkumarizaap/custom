...

{
	...
	$product_color =mysqli_real_escape_string($con,trim($_POST['product_color']));
	...
	$st = !empty($_POST['ps_id']) ? $_POST['ps_id'] :"0";

	$st=!empty($_POST['style']) ? str_replace(array("[","]"),"",json_encode($_POST['style'])) : "0";
...

	{
		...
		$p_rand=substr(strtoupper($product_title), 0,3).rand(0,9999);
		 ...
		 $sql=mysqli_query($con,"insert into product_master(p_rand,p_name,p_color,p_description,p_info,p_price,waist_price,extra_pant,p_default_style,featured,highlighted,highlighted_img,catid,subcatid,fabid,frontend,backend,created_date) 

				VALUES 	('".$p_rand."','".$product_title."','".$product_color."','".$product_desc."','".$product_info."','".$price."','".$waist_price."','".$extra_pant."','".$st."','".$option1."','".$option2."','".$img."','".$category."','".$sub_category."','".$fabric."','".$option2."','".$option3."','".$date."')");
...

	{
		...
		p_price='".$price."',catid='".$category."',subcatid='".$sub_category."',fabid='".$fabric."',last_updated=NOW(),waist_price='".$waist_price."',frontend='".$option2."',p_color='".$product_color."',
...
if($type=="fabrics")
{
	...
	if($action=="save")
	{
		...
		for($a=0;$a<$count;$a++)
		{
			...
			if($_FILES['default_img']['size'][$a]!=0)
			{
				...
				$temp1='';$target1='';$rand1='';$ext1='';
      }		        
      $sql=mysqli_query($con,"insert into fabric_master(fab_rand,fab_name,fab_img,fab_desc,fab_price,catid,default_img,created_date)values('".$ra."','".$fabric_name."','".$img."','".$fabric_desc."','".$fabric_price."','".$category[$a]."','".$img1."','".$date."')");
	...
	else if($action=="update")
	{
		...
		$date=date('Y-m-d H:i:s');
		$sql=mysqli_query($con,"update fabric_master set fab_name='".$fabric_name."',fab_img='".$img."',fab_desc='".$fabric_desc."',fab_price='".$fabric_price."',catid='".$category."',default_img='".$img1."',last_updated='".$date."' where fab_id=$id");
	}
	echo "success";
}
if($type=="user_role")
{
	...
	if($action=="role_add")
	{
		if($usertype=="3"){$group="3";}else if($usertype=="4"){$group="2";}
		$j_date=date("Y-m-d");
		$sql=mysqli_query($con,"insert into member (first_name,last_name,email,join_date)values('".$vendor_name."','".$lastname."','".$primary_email."','".$j_date."')");
...

{
	...
	$city=mysqli_real_escape_string($con,trim($_POST['city']));
	$date=date("Y-m-d H:i:s",strtotime($_POST['created_date']));

	$sql=mysqli_query($con,"update reviews set subcatid='".$category."',name='".$name."',email='".$email."',title='".$title."',state='".$state."',city='".$city."',description='".$description."',score='".$rating."',pid='".$product."',created_date='".$date."' where r_id='".$id."'");
...
else if($type=="property_accents")
{
	$action=mysqli_real_escape_string($con,trim($_POST['action']));
	$product=mysqli_real_escape_string($con,trim($_POST['product_category']));
	if($_POST['jacket_radio']=="2")
		$jacket=mysqli_real_escape_string($con,trim($_POST['jacket_lining']));
	else
		$jacket = "0";
	if($_POST['embroidery_radio']=="2")
		$embroidery=mysqli_real_escape_string($con,trim($_POST['embroidery']));
	else
		$embroidery = "0";
	if($_POST['buttons_radio']=="2")
		$buttons=mysqli_real_escape_string($con,trim($_POST['buttons']));
	else
		$buttons = "0";
	if($_POST['neck_radio']=="2")
		$neck=mysqli_real_escape_string($con,trim($_POST['neck_lining']));
	else
			$neck = "0";
	if($_POST['pocket_radio']=="2")
		$pocket=mysqli_real_escape_string($con,trim($_POST['pocket']));
	else
			$pocket = "0";
	if($_POST['threads_radio']=="2")
		$threads=mysqli_real_escape_string($con,trim($_POST['threads']));
	else
			$threads = "0";
	if($_POST['elbow_radio']=="2")
		$elbow=mysqli_real_escape_string($con,trim($_POST['elbow']));
	else
			$elbow = "0";
	if($_POST['monogram_radio']=="2")
		$monogram=mysqli_real_escape_string($con,trim($_POST['monogram']));
	else
			$monogram = "0";
	if($_POST['cuff_radio']=="2")
		$cuff=mysqli_real_escape_string($con,trim($_POST['cuff']));
	else
			$cuff = "0";
	if($_POST['collar_radio']=="2")
		$collar=mysqli_real_escape_string($con,trim($_POST['collar']));
	else
			$collar = "0";
	$id=mysqli_real_escape_string($con,trim($_POST['id']));
	// if($action=="save")
	// {
	// 	$sql=mysqli_query($con,"insert into accents_master(product_type,jacket_lining_price,embroidery_price,button_price,neck_lining_price,pocket_price,threads_price,elbow_patch_price) values('".$product."','".$jacket."','".$embroidery."','".$buttons."','".$neck."','".$pocket."','".$threads."','".$elbow."')");
	// }
	// else if($action=="update")
	// {
		$sql=mysqli_query($con,"update accents_master set jacket_lining_price='".$jacket."',embroidery_price='".$embroidery."',button_price='".$buttons."',neck_lining_price='".$neck."',pocket_price='".$pocket."',threads_price='".$threads."',elbow_patch_price='".$elbow."',monogram_price='".$monogram."',collar_lining_price='".$collar."',cuff_style_price='".$cuff."' where product_type='".$product."'");	
	// }
	echo "success";
}


else if($type=="custom_color_add")
{
	$name            = $_POST['image_name'];
	$description     = $_POST['description'];
	$price           = $_POST['price'];
	$old_large_image = $_POST['old_large_image'];
	$old_small_image = $_POST['old_small_image'];
	$sub_cat_id      = $_POST['subcat_name'];
	$action   = mysqli_real_escape_string($con,trim($_POST['action']));
	$id       = mysqli_real_escape_string($con,trim($_POST['id']));
	$img      = ""; $small_img_old = ''; $small_img_nw = '';
	$large_img_old = ''; $large_img_nw = '';
	$path     = "../../assets/dimg/lining/default/";
	$path1    ="assets/dimg/lining/default/";
	if(!file_exists($path))
	{
		mkdir($path,0777,true);
	}

	for ($k=0; $k <count($_POST['image_name']) ; $k++) 
	{
		if($_FILES['col_large_img']['size'][$k]!=0)
		{
			//large image upload
			$type      = $_FILES['col_large_img']['type'][$k];
			$ext       = get_image_type($type);
			$rand      = "large"."_".time()."_".$_FILES['col_large_img']['name'][$k];
			$temp      = $_FILES['col_large_img']['tmp_name'][$k];
			$target    = $path.$rand;
			$target1   = $path1.$rand;
			$large_img_nw = $target1;
			move_uploaded_file($temp,$target);
			$temp  ='';
			$target='';
			$rand  ='';
		...
		{
			$large_img_nw = $old_large_image[$k];
		}
		if($_FILES['col_small_img']['size'][$k]!=0)
		{
			//small image upload		
			$type1      = $_FILES['col_small_img']['type'][$k];
			$ext1       = get_image_type($type1);
			$rand1      = "small"."_".time()."_".$_FILES['col_small_img']['name'][$k];
			$temp1      = $_FILES['col_small_img']['tmp_name'][$k];
			$targetSmall= $path.$rand1;
			$target2    = $path1.$rand1;
			$small_img_nw = $target2;
			move_uploaded_file($temp1,$targetSmall);
			$temp1      ='';
			$targetSmall='';
			$rand1      ='';
			$ext1       ='';
		...
		{
			$small_img_nw = $old_small_image[$k];
		}
		if($action=="save")
		{
			$sql=mysqli_query($con,"insert into custom_color_lining set sub_cat_id='".$sub_cat_id."',image_name='".$name[$k]."',large_image_name='".$large_img_nw."',small_image_name='".$small_img_nw."',price='".$price[$k]."',description='".$description[$k]."'");
		}
	}
	if($action=="update")
	{
		$id = $_POST['id'];
		$sql= mysqli_query($con,"update custom_color_lining set sub_cat_id='".$sub_cat_id."',image_name='".$name[0]."',large_image_name='".$large_img_nw."',small_image_name='".$small_img_nw."',price='".$price[0]."',description='".$description[0]."' where id='".$id."'");
	}
...
else if($type=="showroom_template")
{
	$sr_id            = $_POST['showroom_id'];
        $email            = $_POST['email'];
	$body_message     = $_POST['body_message'];
	$bottom_message		= $_POST['bottom_message'];
	$action   = mysqli_real_escape_string($con,trim($_POST['action']));
	// $sql=mysqli_query($con,"insert into showroom_email_template(sr_id,email,body_message,bottom_message) values('".$sr_id."','".$email."','".$body_message."','".$bottom_message."')");
	$sql=mysqli_query($con,"update showroom_email_template set email='".$email."',body_message='".$body_message."',bottom_message='".$bottom_message."'
		 where sr_id='".$sr_id."'");
}
...
function get_fab_rand()
{
	$sql = mysqli_query($con,"select max(fab_rand) + 1 as rand from fabric_master");
	$r = mysqli_fetch_array($sql);
	return $r['rand'];
}

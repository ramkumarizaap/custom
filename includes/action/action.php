...
if(isset($_POST['apt_form']))
{
	$name=mysqli_real_escape_string($con,trim($_POST['name']));
	$email=mysqli_real_escape_string($con,trim($_POST['email']));
	$type=mysqli_real_escape_string($con,trim($_POST['appointment_type']));
	...
	$sql1=mysqli_query($con,"select * from showroom_master where sr_id=$sr_id");
	$r1=mysqli_fetch_array($sql1);
	$stitle=$r1['sr_title'];
	$sr_address=$r1['sr_address'];
	$sdate = date("Y-m-d",strtotime($sdate));
	$timings = date("H:i:s",strtotime($timings));
	$sql=mysqli_query($con,"select * from appointment_master where srid=$sr_id and a_date='".$sdate."' and a_time='".$time."'");
	...
	{
		$sql1=mysqli_query($con,"insert into appointment_master(a_name,a_email,a_type,a_phone,a_date,a_time,a_address,a_comments,srid,created_date)
			values('".$name."','".$email."','".$type."','".$phone."','".$sdate."','".$timings."','".$address."','".$comments."','".$sr_id."','".$date."')");
		$sql4=mysqli_query($con,"select * from showroom_email_template where sr_id=$sr_id");
		$r4 = mysqli_fetch_array($sql4);
		$search = array("{name}","{email}","{location}","{showroom}","{address}","{comments}","{phone}","{apptype}","{timings}");
		$s_time = date("m/d/Y H:i",strtotime($sdate." ".$timings));
		$replace = array($name,$email,$sr_address,$stitle,$address,$comments,$phone,$type,$s_time);
		$message = str_replace($search, $replace,$r4['body_message']);
		$message .= "<br><br>".$r4['bottom_message'];
                $showroom_email = $r4['email'];
    $subject	=	"New Appointment";
		$subject1	=	"Appointment Booked";
    $headers = "From: $stitle <".$r4['email']."> \r\n";
    $headers .= "MIME-Version: 1.0\r\n";
    $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
    $message1 = "New appointment has been booked by $name ($email) - <b>$phone</b> for $type at $s_time.<br><br><b>Customer Address :</b> <br>$address<br><br><br>Comments :<br>$comments.";
    mail($email,$subject1,$message,$headers);
    mail("ramkumar.izaap@gmail.com",$subject,$message1,$headers);
		echo "Success";
	}
}
...
if(isset($_POST['section']) && $_POST['section']=='style') {
  ...
  $_SESSION['suit']['accents'] = array('jacket_lining_type' => isset($_POST['lining_jacket_radio']) ? $_POST['lining_jacket_radio'] : '',
            ...
            'colored_holes_type' => isset($_POST['button_holes_threads_jacket1']) ? $_POST['button_holes_threads_jacket1']:'',
  					'embroidery_type'=> isset($_POST['embroidery_type']) ? $_POST['embroidery_type']:'',
  					'embroidery_price' => !empty($_SESSION['customizer_price']['embroidery']) ? $_SESSION['customizer_price']['embroidery']:'0');
...
if(isset($_POST['section']) && $_POST['section']=='style') {
  ...
  $_SESSION['jacket']['accents'] = array('jacket_lining_type' => isset($_POST['lining_jacket_radio']) ? $_POST['lining_jacket_radio'] : '',
              'lining_price' => isset($_POST['lining_jacket_price']) ? $_POST['lining_jacket_price']:'',
              'tot_price'=> (!empty($_SESSION['customizer_price']['base']) ? $_SESSION['customizer_price']['base']:'0') + (!empty($_SESSION['customizer_price']['waistcoat']) ? $_SESSION['customizer_price']['waistcoat']:'0') + (!empty($_SESSION['customizer_price']['fabric']) ? $_SESSION['customizer_price']['fabric']:'0') + (!empty($_SESSION['customizer_price']['metal_buttons']) ? $_SESSION['customizer_price']['metal_buttons']:'0') + (!empty($_SESSION['customizer_price']['patches']) ? $_SESSION['customizer_price']['patches']:'0') + (!empty($_SESSION['customizer_price']['handkerchief']) ? $_SESSION['customizer_price']['handkerchief']:'0') + (!empty($_SESSION['customizer_price']['button_holes_threads_jacket']) ? $_SESSION['customizer_price']['button_holes_threads_jacket']:'0')+(!empty($_SESSION['customizer_price']['initials_jacket']) ? $_SESSION['customizer_price']['initials_jacket']:'0')+(!empty($_SESSION['customizer_price']['lining_jacket']) ? $_SESSION['customizer_price']['lining_jacket']:'0')+(!empty($_SESSION['customizer_price']['embroidery']) ? $_SESSION['customizer_price']['embroidery']:'0')+(!empty($_SESSION['customizer_price']['neck_lining']) ? $_SESSION['customizer_price']['neck_lining']:'0'),
              'embroidery_type' => isset($_POST['embroidery_type']) ? $_POST['embroidery_type']:'',
              'embroidery_price' =>!empty($_SESSION['customizer_price']['embroidery']) ? $_SESSION['customizer_price']['embroidery']:'0',
              'jacket_lining_id' => isset($_POST['jacket_lining_id']) ? $_POST['jacket_lining_id']:'',
              'font_price' => isset($_POST['initials_jacket_price']) ? $_POST['initials_jacket_price']:'',
  ...
  if($go_to!='extras') {
      if($go_to == "configure")
   	...
   	// print_r($_SESSION['jacket']['accents']);
   	// exit;
        $gid=isset($_SESSION['guest_id']) ? $_SESSION['guest_id'] :"0";
        $user_id=isset($_SESSION['user_id']) ? $_SESSION['user_id'] :"0";
...
if(isset($_POST['section']) && $_POST['section']=='style') {
  ...
  $_SESSION['coat']['accents'] = array('jacket_lining_type' => isset($_POST['lining_jacket_radio']) ? $_POST['lining_jacket_radio'] : '',
              'lining_price' => isset($_POST['lining_jacket_price']) ? $_POST['lining_jacket_price']:'',
              'tot_price'=> (!empty($_SESSION['customizer_price']['base']) ? $_SESSION['customizer_price']['base']:'0') + (!empty($_SESSION['customizer_price']['waistcoat']) ? $_SESSION['customizer_price']['waistcoat']:'0') + (!empty($_SESSION['customizer_price']['fabric']) ? $_SESSION['customizer_price']['fabric']:'0') + (!empty($_SESSION['customizer_price']['metal_buttons']) ? $_SESSION['customizer_price']['metal_buttons']:'0') + (!empty($_SESSION['customizer_price']['patches']) ? $_SESSION['customizer_price']['patches']:'0') + (!empty($_SESSION['customizer_price']['handkerchief']) ? $_SESSION['customizer_price']['handkerchief']:'0') + (!empty($_SESSION['customizer_price']['button_holes_threads_jacket']) ? $_SESSION['customizer_price']['button_holes_threads_jacket']:'0') + (!empty($_SESSION['customizer_price']['neck_lining']) ? $_SESSION['customizer_price']['neck_lining']:'0')+ (!empty($_SESSION['customizer_price']['initials']) ? $_SESSION['customizer_price']['initials']:'0')+ (!empty($_SESSION['customizer_price']['lining_jacket']) ? $_SESSION['customizer_price']['lining_jacket']:'0'),
              'neck_lining' => !empty($_SESSION['customizer_price']['neck_lining']) ? $_SESSION['customizer_price']['neck_lining']:'0',
              'jacket_lining_id' => isset($_POST['jacket_lining_id']) ? $_POST['jacket_lining_id']:'',
              'embroidery_type' => isset($_POST['embroidery_type']) ? $_POST['embroidery_type']:'',
              'embroidery_price' => isset($_POST['embroidery_price']) ? $_POST['embroidery_price']:'',
              'font_price' => isset($_POST['initials_jacket_price']) ? $_POST['initials_jacket_price']:'',
              'font_family' => isset($_POST['font_family']) ? $_POST['font_family']:'',
              'initials_jacket' => isset($_POST['initials_jacket']) ? $_POST['initials_jacket']:'',
              'monogram_color' => isset($_POST['initials_jacket1']) ? $_POST['initials_jacket1']:'',
              ...
              'type_of_colored_button_holes' => isset($_POST['button_holes_threads_jacket']) ? $_POST['button_holes_threads_jacket']:'',
              'lapel_color' => isset($_POST['placket_color']) ? $_POST['placket_color']:'',
              'button_holes_price' => !empty($_POST['button_holes_threads_price']) ? $_POST['button_holes_threads_price']:'0',
              'colored_thread_type' => isset($_POST['button_holes_threads_jacket2']) ? $_POST['button_holes_threads_jacket2']:'',
              'colored_holes_type' => isset($_POST['button_holes_threads_jacket1']) ? $_POST['button_holes_threads_jacket1']:'');
// print_r($_SESSION['coat']['accents']);
//         exit;
  $go_to = $_POST['go_to'];
  $action = $_POST['action'];
   	...
   	// exit;
        ...
        if(isset($_SESSION['user_id'])){
         ...
         if($action=="save")
         {
            ...
            $sql=mysqli_query($con,"insert into order_master(order_id,pid,userid,sess_id,subcatid,p_type,om_quantity,om_price,om_style,om_fab,om_accents,order_status,created_date,last_updated)
             values('".$order_id."','".$p_id."','".$user_id."','".$gid."',5,'".$p_type."',1,'".$om_price."','".$style."','".$fabric."','".$accents."','Processing','".$date."','".$date."')");            
...
if(isset($_POST['section']) && $_POST['section']=='style') {
  ...
  $_SESSION['shirt']['accents'] = array('tot_price'=> (!empty($_SESSION['customizer_price']['base']) ? $_SESSION['customizer_price']['base']:'0') + (!empty($_SESSION['customizer_price']['fabric']) ? $_SESSION['customizer_price']['fabric']:'0') + (!empty($_SESSION['customizer_price']['neck_lining']) ? $_SESSION['customizer_price']['neck_lining']:'0') + (!empty($_SESSION['customizer_price']['patches_jacket']) ? $_SESSION['customizer_price']['patches_jacket']:'0') + (!empty($_SESSION['customizer_price']['handkerchief']) ? $_SESSION['customizer_price']['handkerchief']:'0') + (!empty($_SESSION['customizer_price']['button_holes_threads_jacket']) ? $_SESSION['customizer_price']['button_holes_threads_jacket']:'0')+ (!empty($_SESSION['customizer_price']['initials_jacket']) ? $_SESSION['customizer_price']['initials_jacket']:'0'),
  	'monogram_price' => !empty($_SESSION['customizer_price']['initials_jacket']) ? $_SESSION['customizer_price']['initials_jacket']:'0',
              'font_price' => isset($_POST['initials_jacket_price']) ? $_POST['initials_jacket_price']:'',
              'monogram_type' => isset($_POST['monogram_type']) ? $_POST['monogram_type']:'',
              'font_family' => isset($_POST['font_family']) ? $_POST['font_family']:'',
              'initials_jacket' => isset($_POST['initials_jacket']) ? $_POST['initials_jacket']:'',
              'monogram_color' => isset($_POST['initials_jacket1']) ? $_POST['initials_jacket1']:'',
              'neck_styling' => isset($_POST['neck_type']) ? $_POST['neck_type']:'',
              'neck_lining_price' => !empty($_POST['neck_lining_price']) ? $_POST['neck_lining_price']: '0',
              ...
              'cuff_lining_price' => !empty($_POST['handkerchief_price']) ? $_POST['handkerchief_price']: '0',
              'cuff_color' => isset($_POST['cuff_color']) ? $_POST['cuff_color']:'',
              'type_of_elbow' => isset($_POST['patches_type']) ? $_POST['patches_type']:'',
              'elbow_price' => !empty($_POST['patches_price']) ? $_POST['patches_price']: '0',
              'elbow_type' => isset($_POST['patches']) ? $_POST['patches']:'',
              'type_of_colored_button_holes' => isset($_POST['button_holes_threads_jacket']) ? $_POST['button_holes_threads_jacket']:'',
              'button_holes_price' => !empty($_POST['button_holes_threads_price']) ? $_POST['button_holes_threads_price']: '0',
              'colored_thread_type' => isset($_POST['button_holes_threads_jacket2']) ? $_POST['button_holes_threads_jacket2']:'',
  ...
  if($go_to!='extras') {
      if($go_to == "configure")
   	...
   	// echo "<pre>";
   	// print_r($_SESSION['customizer_price']);
   	// print_r($_SESSION['shirt']['accents']);
   	// exit;
        $gid=isset($_SESSION['guest_id']) ? $_SESSION['guest_id'] :"0";
        $user_id=isset($_SESSION['user_id']) ? $_SESSION['user_id'] :"0";
...
if($_POST['type']=="update-apt")
{
	...
	$t=$date. ' '.$time1;
	$sql=mysqli_query($con,"select * from appointment_master where a_email!='".$email."' and a_date='".$date."' and
		a_time='".$time1."' and srid=$srid");
	if(mysqli_num_rows($sql))
	{
		echo "Fail";
	}
	...
	{
		$sql1=mysqli_query($con,"update appointment_master set a_date='".$date."',a_time='".$time1."' where a_id=$aid");
		echo "Success";
	}

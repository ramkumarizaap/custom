...
function get_all_reviews($st='',$id='')
    ...
    function get_custom_color_lining($id='', $subcat_id='')
    {
        global $con;$res=array();$i=0; $where = '';
        ...
        if(!empty($subcat_id)){
            $where .= (!empty($subcat_id))?" and sub_cat_id='".$subcat_id."'":"";   
        }

        $sql=mysqli_query($con,"select * from custom_color_lining where 1=1 $where order by created_date asc");

            if(mysqli_num_rows($sql) > 0) {
                while($r=mysqli_fetch_array($sql)){
                    
                    $res[$i]['id']              = $r['id'];
                    $res[$i]['sub_cat_id']      = $r['sub_cat_id'];
                    $res[$i]['name']            = $r['image_name'];
                    $res[$i]['description']            = $r['description'];
                    $res[$i]['small_image_name']= $r['small_image_name'];
                    $res[$i]['large_image_name']= $r['large_image_name'];
                    $res[$i]['price']           = $r['price'];
                    $res[$i]['date']            = $r['created_date'];
                    $i++;
                }
    ...
    function subCategories() 
    {
        ...
        $sql = mysqli_query($con,"select a.*,b.cat_name from sub_category_master a inner join category_master b on a.catid=b.cat_id where 1=1");
